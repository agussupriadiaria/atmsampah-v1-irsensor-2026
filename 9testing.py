...
def mainPage():
    ...
    # Set window geometry ke full screen size
    ...
    # Hide cursor
    root.config(bg="white", cursor="none")
    
    # Set title (for logging purposes)
    root.title("ATM Sampah - PilahSampah")
    
    # Update window
    root.update()
    
    print("[DISPLAY] Fullscreen enabled - kiosk mode")

    # === BACKGROUND COLOR (SIMPLE, NO IMAGE) ===
    bg_main_frame = Frame(root, bg="white")
    bg_main_frame.place(x=0, y=0, relwidth=1, relheight=1)
    
    # Title
    titleLabel = Label(bg_main_frame, text="PILAHSAMPAH", font=("Helvetica", 20, "bold"), bg="white")
